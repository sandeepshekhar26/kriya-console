...
/**
 * Control plane — the on-prem kriyad aggregator dashboard (the paid upsell), DEMO build only. Shows the
 * fleet's coverage and lets an auditor RE-PROVE a device's signed evidence offline, trusting neither the
 * device nor this server. The re-proof + adversarial catches run the SAME in-browser Ed25519 verifier the
 * trust spine parity-tests against Rust — so the green checks and red catches are real cryptography, not
 * theatre. This module is dynamically imported only behind `__KRIYA_DEMO__`, so its seeded fleet data is
 * tree-shaken out of the shipped desktop app (which shows the empty "No aggregator connected" state).
 */
